...
class GetItemView(APIView):
    def get(self, request, format=None):
        ...
        try:
            ...
            return Response({'error':'Something went wrong when retrieving wishlist item'},status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class AddItemView(APIView):
    def post(self, request, format=None):
        ...
        try:
            ...
            if WishListItem.objects.filter(product=product,user=user).exists():
            ...
            return Response({'error':'Something went wrong when adding item to wishlist'},status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class GetItemTotalView(APIView):
    def get(self, request, format=None):
        ...
        try:
            ...
            return Response({'error': "something went wrong retrieving total number of wishlist items"},status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class RemoveItemView(APIView):
    def delete(self,request,format=None):
        ...
        try:
            ...
            product=Product.objects.get(id=product_id)
            if not WishList.objects.filter(wishlist=wishlist,product=product).exists():
                return Response({'error':'This product is not in your wishlist'},status=status.HTTP_404_NOT_FOUND)
            ...
            return Response({'error': 'Something went wrong when removing wishlist item'},status=status.HTTP_500_INTERNAL_SERVER_ERROR)
