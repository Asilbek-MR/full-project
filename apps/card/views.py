...
class GetItemsView(APIView):
    def get(self,request,format=None):
        ...
        try:
            ...
            return Response(
                {'error': 'Something went wrong when retrieving cart item'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def post(self, request, format=None):
        ...
        try:
            if not Product.objects.filter(id=product_id).exists():
                return Response(
                    {'error': 'This product does not exist'},status=status.HTTP_404_NOT_FOUND)
            ...
            return Response(
                {'error': 'Something went wrong when adding item to cart'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class GetTotalView(APIView):
    def get(self, request, format=None):
        ...
        try:
            ...
            total_cost = 0.0
            total_compere_cost=0.0

            if cart_items.exists():
                for cart_item in cart_items:
                    total_cost += (float(cart_item.product.price) * float(cart_item.count))
                    total_compere_cost+=(float(cart_item.product.compare_price)*float(cart_item.count))
                total_cost=round(total_cost,2)
                total_compere_cost=round(total_compere_cost,2)
            return Response({'total_cost':total_cost,'total_compere_cost':total_compere_cost},status=status.HTTP_200_OK)
        except:
...
class UpdateItemView(APIView):
    def put(self,request,format=None):
        ...
        try:
            if not Product.objects.filter(id=product_id).exists():
                Response({'error':'This product does not exist'},status=status.HTTP_404_NOT_FOUND)
            product = Product.objects.get(id=product_id)
            cart=Card.objects.get(user=user)
            ...
            return Response({'error': 'Something went wrong when updating cart item'})
...
class RemoveItemView(APIView):
    def delete(self,request,format=None):
        ...
        try:
            ...
            if not CardItem.objects.filter(cart=cart,product=product).exists():
                return Response({'error':'This product is not in your card'},status=status.HTTP_404_NOT_FOUND)
            ...
            return Response({'error': 'Something went wrong when removing item'}, status=status.HTTP_200_OK)
...
class EmptyCartView(APIView):
    def delete(self, request, format=None):
        ...
        try:
            ...
            CardItem.objects.filter(user=user).delete()
    ...
    def put(self,request,format=None):
        ...
        try:
            ...
            for cart_item in cart_items:
                ...
                if not Product.objects.filter(cart=cart,product=product).exists():
                    item=Product.objects.filter(cart=cart,product=product)
                    count=item.count
                    ...
                    if (cart_item_count + int(count)) <= int(quantity):
                        ...
                        CardItem.objects.filter(cart=cart,product=product).update(count=update_count)
                else:
                    try:
                        cart_item_count=int(cart_item['count'])
                    except:
                    ...
                    if cart_item_count <= quantity:
                        CardItem.objects.create(product=product,cart=cart,count=cart_item_count)
                        if CardItem.objects.filter(cart=cart).exists():
                            total_items=int(cart.total_items) + 1
                            Card.objects.filter(user=user).update(total_items=total_items)
                return Response({'success':'Card Synchronized'},status=status.HTTP_200_OK)
        except:
            return Response({'error': 'Something went wrong when synching cart'},status=status.HTTP_200_OK)
