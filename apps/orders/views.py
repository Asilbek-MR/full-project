    ...
    def get(self, request, transactionId, format=None):
        ...
        try:
            if Order.objects.filter(user=user, transaction_id=transactionId).exists():
                ...
                return Response({'error': 'Order with this transaction ID does not exist'},status=status.HTTP_200_OK)
        except:
            return Response({'error':'Something went wrong when retrieving order email'},status=status.HTTP_500_INTERNAL_SERVER_ERROR)
