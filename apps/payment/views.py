...
from apps.card.models import Card,CardItem
...
class GetPaymentTotalView(APIView):
    def get(self, request,format=None):
        ...
        try:
            ...
            if not CardItem.objects.filter(card=card).exists():
                return Response({'error':'Need to have items in cart'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            
            cart_items=CardItem.objects.filter(card=card)

            for cart_item in cart_items:
                if not Product.objects.filter(id=cart_item.product.id).exists():
                    return Response({'error':"A product with ID provided does not exist"},status=status.HTTP_404_NOT_FOUND)
                
                if int(cart_item.count) > int(cart_item.product.quantity):
                    return Response({'error': 'No enough items in stock'},status=status.HTTP_404_NOT_FOUND)
                ...
                estimate_tax=round(total_amount*tax,2)
                ...
                return Response({
                    ...
                    'estimated_tax': f'{estimate_tax:.2f}',
                    'shipping_cost':f'{shipping_cost:.2f}'
                    },status=status.HTTP_200_OK)
        except:
            return Response({'error': 'Something went wrong when retrieving payment total information'},status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class ProcessPaymentView(APIView):
    def post(self,request,format=None):
        ...
        if not Shipping.objects.filter(id__iexact=shipping_id).exists():
            return Response({'error': 'Invalid shipping option'},status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        
        cart=Card.objects.get(user=user)

        if not CardItem.objects.filter(cart=cart).exists():
            return Response({'error': 'Need to have items in card'}, status=status.HTTP_404_NOT_FOUND)

        cart_items=CardItem.objects.filter(cart=cart)

        for cart_item in cart_items:
            if not Product.objects.filter(id=cart_item.product.id).exists():
                return Response({'error': 'Transaction failed , a product ID does not exist'}, status=status.HTTP_404_NOT_FOUND)
            
            if int(cart_item.count) > int(cart_item.product.quantity):
                return Response({'error':'Not enough items in stock'}, status=status.HTTP_200_OK)
