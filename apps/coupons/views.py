    ...
    def get(self, request,format=None):
        try:
            ...
            if FixedPriceCoupon.objects.filter(name=coupon_name).exists():
                ...
                return Response({'error': 'Coupon code not found'},status=status.HTTP_200_OK)
        except:
            return Response({'error': 'Something went wrong when checking coupon'},status=status.HTTP_200_OK)
