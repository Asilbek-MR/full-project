...
class GetProductReviewsView(APIView):
    ...
    def get(self, request, productId, format=None):
        ...
        try:
            ...
            return Response({'error':'Something went wrong when retrieving reviews'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class CreateProductReviewView(APIView):
    def post(self, request,productId,format=None):
        ...
        try:
            ...
            if Review.objects.filter(user=user, product=product).exists():
                return Response({'error':'Review for this course already created'},status=status.HTTP_409_CONFLICT)
            ...
            if Review.objects.filter(user=user,product=product).exists():
                ...
                for review in reviews:
                    ...
                    results.append(item)
            return Response({'review': result,'reviews':reviews},status=status.HTTP_200_OK)
        except:
            return Response({'error': 'Something went wrong when creating review'},status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class UpdateProductReviewView(APIView):
    def put(self,request , productId,format=None):
        ...
        try:
            ...
            return Response({'error':'Must pass a comment when creating review'},status=status.HTTP_400_BAD_REQUEST)
        try:
            if not Review.objects.filter(id=product_id).exists():
                return Response({'error': 'This product does not exist'},status=status.HTTP_404_NOT_FOUND)
            ...
            if not Review.objects.filter(user=user,product=product).exists():
                return Response({'error':'Review for this product does not exist'},status=status.HTTP_404_NOT_FOUND)
            ...
            if Review.objects.filter(user=user,product=product).exists():
                ...
                result['rating']=review.rating
                result['comment']=review.comment
                result['date_crated']=review.date_created
                result['user']=review.user.first_name
            ...
            return Response({'error':'Something went wrong when updating review'},status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class DeleteProductReviewView(APIView):
    def delete(self,request,productId,format=None):
        ...
        try:
            ...
            return Response({'error':'Something went wrong when deleting product review'},status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class FilterProductReviewsView(APIView):
    ...
    def get(self,request,productId,format=None):
        ...
        try:
            ...
            return Response({'error':'Something went wrong when filtering reviews for product'},status=status.HTTP_500_INTERNAL_SERVER_ERROR)                    
